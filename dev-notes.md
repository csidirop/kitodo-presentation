...
## Manual Backend Setup Notes

The current viewer/config setup can also be assembled manually in the TYPO3 backend, but only if all required pieces are created consistently. This was verified in the backend after the CLI and backend-module based setup flows were already in place.

### What manual creation means in practice

Creating only these page records is not sufficient on its own:
- a root page
- a `Viewer` page
- a `Kitodo Configuration` folder

The viewer only works if the structural pages are combined with:
- a TYPO3 site configuration for the root page
- a `sys_template` record on the root page
- the correct static TypoScript includes
- the correct TypoScript constants
- tenant defaults in the configuration folder

### Verified manual backend recipe

The working manual sequence is:
...
5. In that record `Edit the whole TypoScript record` and under the Tab `General`:
   ...
   - set the `Constants` field to the required values:
      ...
      plugin.tx_dlf.bootstrap.rootPid = <uid of root page>
      plugin.tx_dlf.bootstrap.viewerPid = <uid of Viewer page>
      ```
   - Under the the tab `Advanced Options`:
      - enable `Rootlevel`
      - Include TypoScript sets:
        - `Basic Configuration`
        - `Install Bootstrap Viewer`
6. Apply tenant defaults to the configuration folder, either through the backend new-tenant module

### Important correction: what to look for in the template record

The bootstrap service does not use older “template module naming” assumptions. The extension registers static TypoScript includes in:

- `Configuration/TCA/Overrides/sys_template.php`

The labels to select in the backend are:
...
Manual backend assembly is possible. The minimum working setup still requires:

- site configuration
- root template
- static TypoScript includes
- correct constants
- tenant data in the configuration folder

That means the current root-setup and tenant-setup flows are still the safer default, but the backend can reproduce the result if these constraints are followed exactly.
