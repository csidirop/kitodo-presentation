...
class FullTextGenerator {
  ...
  protected static function generatePageOCR($ext_key, $conf, $doc, $image_url, $page_num, $sleep_interval = 0) { 
    /* DEBUG */ if($conf['ocrDebugBackend']) echo '<script>alert("BE:FullTextGen.genPageOCR")</script>'; //DEBUG
    $page_id = self::getPageLocalId($doc, $page_num);
    ...
    /* DEBUG */ if($conf['ocrDebugBackend']) echo '<script>alert("BE:FullTextGen.genPageOCR: ocrDummyText: ' . $conf['ocrDummyText'] . '")</script>'; //DEBUG

    if ($conf['ocrDummyText']) {
      /* DEBUG */ if($conf['ocrDebugBackend']) echo '<script>alert("BE:FullTextGen.genPageOCR: ocrDummyText true")</script>'; //DEBUG
      ...
      /* DEBUG */ if($conf['ocrDebugBackend']) echo '<script>alert("BE:FullTextGen.genPageOCR: ocrDummyText false")</script>'; //DEBUG
