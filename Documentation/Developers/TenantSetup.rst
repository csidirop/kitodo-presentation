...
Structures and metadata are created in the default site language first. If the site defines additional languages, the setup also creates translated labels for those records.
If no TYPO3 site can be resolved for the configuration folder, the setup falls back to the default language only.
